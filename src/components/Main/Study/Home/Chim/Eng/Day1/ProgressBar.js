...
export function ProgressBar({
  ...
  const day1 = useDay1();
  const currentStep = day1?.currentStep ?? propCurrentStep ?? 1;
  const subProgresses = day1?.subProgresses ??
    ...
    >
      ...
      >
        {displayProgresses.map((progress, idx) => {
          // 첫 번째 바(학습)는 퀴즈 진행 상황에 따라 3분할 진행
          // 나머지 바들은 이전 단계들이 가득 차게
          let effectiveWidth;
          if (idx === 0) {
            // 학습 바: 퀴즈 진행 상황에 따라 3분할 진행
            if (currentStep >= 2) {
