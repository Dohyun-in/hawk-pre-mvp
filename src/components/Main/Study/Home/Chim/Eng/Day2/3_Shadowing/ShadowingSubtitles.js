import React from "react";

// 나눔고딕 폰트 적용
const EN_FONT =
  "'NanumGothic', 'Pretendard', 'Noto Sans KR', Arial, sans-serif";

// 24방향, 최대 2px, 소수점 포함
const solidWhite24 = [
  "2px 0 0 white",
  "-2px 0 0 white",
  "0 2px 0 white",
  "0 -2px 0 white",
  "1.4px 1.4px 0 white",
  "-1.4px 1.4px 0 white",
  "1.4px -1.4px 0 white",
  "-1.4px -1.4px 0 white",
  "1px 1.7px 0 white",
  "-1px 1.7px 0 white",
  "1px -1.7px 0 white",
  "-1px -1.7px 0 white",
  "1.7px 1px 0 white",
  "-1.7px 1px 0 white",
  "1.7px -1px 0 white",
  "-1.7px -1px 0 white",
  "0.5px 1.9px 0 white",
  "-0.5px 1.9px 0 white",
  "0.5px -1.9px 0 white",
  "-0.5px -1.9px 0 white",
  "1.9px 0.5px 0 white",
  "-1.9px 0.5px 0 white",
  "1.9px -0.5px 0 white",
  "-1.9px -0.5px 0 white",
].join(", ");

const solidBlue24 = [
  "2px 0 0 rgb(68,0,255)",
  "-2px 0 0 rgb(68,0,255)",
  "0 2px 0 rgb(68,0,255)",
  "0 -2px 0 rgb(68,0,255)",
  "1.4px 1.4px 0 rgb(68,0,255)",
  "-1.4px 1.4px 0 rgb(68,0,255)",
  "1.4px -1.4px 0 rgb(68,0,255)",
  "-1.4px -1.4px 0 rgb(68,0,255)",
  "1px 1.7px 0 rgb(68,0,255)",
  "-1px 1.7px 0 rgb(68,0,255)",
  "1px -1.7px 0 rgb(68,0,255)",
  "-1px -1.7px 0 rgb(68,0,255)",
  "1.7px 1px 0 rgb(68,0,255)",
  "-1.7px 1px 0 rgb(68,0,255)",
  "1.7px -1px 0 rgb(68,0,255)",
  "-1.7px -1px 0 rgb(68,0,255)",
  "0.5px 1.9px 0 rgb(68,0,255)",
  "-0.5px 1.9px 0 rgb(68,0,255)",
  "0.5px -1.9px 0 rgb(68,0,255)",
  "-0.5px -1.9px 0 rgb(68,0,255)",
  "1.9px 0.5px 0 rgb(68,0,255)",
  "-1.9px 0.5px 0 rgb(68,0,255)",
  "1.9px -0.5px 0 rgb(68,0,255)",
  "-1.9px -0.5px 0 rgb(68,0,255)",
].join(", ");

const subtitleStyles = {
  en: (isHighlighted) => ({
    position: "absolute",
    bottom: "60px",
    width: "100%",
    fontSize: "24px",
    fontWeight: "700",
    fontFamily: EN_FONT,
    color: isHighlighted ? "rgb(68, 0, 255)" : "#a5b4fc",
    textShadow: isHighlighted ? solidWhite24 : solidBlue24,
    textAlign: "center",
    userSelect: "none",
    pointerEvents: "none",
    whiteSpace: "pre-wrap",
    zIndex: 2,
    lineHeight: "1.4",
    letterSpacing: "-0.5px",
    padding: "0 0px",
  }),
};

export function ShadowingSubtitles({
  subtitles,
  currentSentenceIndex,
  currentTime,
  showEnglish,
  voiceResult,
  similarity,
}) {
  // 음성 인식 결과를 단어별로 분석
  const getCorrectWords = () => {
    if (!voiceResult || similarity < 0.8) return new Set();

    const clean = (str) =>
      str
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, "")
        .split(" ")
        .filter(Boolean);

    const currentSentence = subtitles.enSentences[currentSentenceIndex];
    if (!currentSentence) return new Set();

    const targetWords = clean(currentSentence.text);
    const recognizedWords = clean(voiceResult);

    // 정확히 일치하는 단어들 찾기
    const correctWords = new Set();
    recognizedWords.forEach((word) => {
      if (targetWords.includes(word)) {
        correctWords.add(word);
      }
    });

    return correctWords;
  };

  // 영어 자막 문장+누적 하이라이트 렌더링 함수
  const renderSentence = (sentences, words, currentTime) => {
    if (!sentences || !words) return null;
    const currentSentence = sentences[currentSentenceIndex];
    if (!currentSentence) return null;

    const sentenceWords = words.filter(
      (w) => w.start >= currentSentence.start && w.start <= currentSentence.end
    );

    const correctWords = getCorrectWords();

    return (
      <div style={subtitleStyles.en(false)}>
        {sentenceWords.map((w, idx) => {
          const isCorrect = correctWords.has(
            w.word.toLowerCase().replace(/[^a-z0-9]/g, "")
          );

          return (
            <span
              key={idx}
              style={{
                ...(currentTime >= w.start
                  ? {
                      color: "rgb(68, 0, 255)",
                      textShadow: solidWhite24,
                    }
                  : {
                      color: "#a5b4fc",
                      textShadow: solidBlue24,
                    }),
                // 정확한 발화 시 핑크색 박스 효과
                ...(isCorrect && {
                  backgroundColor: "rgb(255, 206, 216)",
                  borderRadius: "6px",
                  paddingTop: "0px",
                  paddingRight: "0px",
                  paddingBottom: "2px",
                  paddingLeft: "3px",
                  marginTop: "1px", // 위쪽 마진
                  marginRight: "1px", // 오른쪽 마진 유지
                  marginBottom: "0px", // 아래쪽 마진
                  marginLeft: "0px", // 왼쪽 마진
                  color: "#a80000", // 정확한 발화 시 빨간색으로 변경
                  //textShadow: "none", // 그림자 제거
                }),
              }}
            >
              {w.word + " "}
            </span>
          );
        })}
      </div>
    );
  };

  const renderSubtitle = () => {
    if (!subtitles.enSentences || !subtitles.en) return null;
    return renderSentence(subtitles.enSentences, subtitles.en, currentTime);
  };

  return <>{showEnglish && renderSubtitle()}</>;
}
