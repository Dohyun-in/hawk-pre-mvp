...
export function Prelearn() {
  ...
  const { updateSubProgress } = useDay2();

  // 퀴즈 진행률을 Day2Context에 업데이트
  useEffect(() => {
    const totalQuizzes = quizzes.length; // 이제 4개
    const currentProgress = (page + 1) / totalQuizzes; // 0부터 시작하므로 +1
    console.log(
      `Day2 퀴즈 진행률 업데이트: page=${page}, progress=${currentProgress}`
    );
